...
const scripts = `"start": "webpack-dev-server --port 8080 --open --mode=development",
...
const node_engine = `"engines": ${JSON.stringify(packageJson.engines)}`;
...
exec(`mkdir ${process.argv[2]} && cd ${process.argv[2]} && npm init -f`, (initErr) => {
  ...
  exec(
    `cd ${process.argv[2]} && node -v && npm -v && npm i -D ${devDeps} && npm i -S ${deps}`,
    (npmErr, npmStdout) => {
      // npmStderr
      if (npmErr) {
